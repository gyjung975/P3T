...
@torch.no_grad()
class ULIP_PointBERT(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        backbone_ckpt = 'data/pretrained_models/pointbert_ulip2.pt' if self.args.ulip2 else 'data/pretrained_models/pointbert_ulip.pt'
        if not self.args.evaluate_3d:
            self.load_model_from_ckpt(backbone_ckpt)

    def load_model_from_ckpt(self, bert_ckpt_path):
        ckpt = torch.load(bert_ckpt_path, map_location=torch.device('cpu'))
        base_ckpt = {k.replace("module.", "").replace("point_encoder.", ""): v for k, v in ckpt['state_dict'].items() if 'point_encoder' in k}
        ...
        print_log(f'[Feature extractor] Successful Loading the ckpt from {bert_ckpt_path}', logger='Feature extractor')
