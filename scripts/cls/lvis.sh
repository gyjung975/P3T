...
dataset=objaverse_lvis_colored
npoints=1024
...
exp_name=${task}-lvis-${context_len}v-${class_pos}-\
...
python -m torch.distributed.launch --nproc_per_node=${ngpu} ${main_program} --proj_name ${proj_name} --main_program ${main_program} --task ${task} \
    ...
    --exp_name ${exp_name} --output_dir ${output_dir} --print_freq ${print_freq} \
...
#torchrun --nnodes=1 --master_port=29501 --nproc_per_node=${ngpu} ${main_program} --proj_name ${proj_name} --main_program ${main_program} --task ${task} \
#    --model ${model} --dataset_name ${dataset} --npoints ${npoints} \
#    --optim ${optim} --lr ${lr} --label_smoothing ${smooth} --epochs ${epochs} --batch_size ${batch_size} \
#    --exp_name ${exp_name} --output_dir ${output_dir} --print_freq ${print_freq} \
#    --num_learnable_prompt_tokens ${context_len} --class_name_position ${class_pos} \
