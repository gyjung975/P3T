...
exp_name=${task}-sonn_objonly-${context_len}v-${class_pos}-${nshots}s${seed}-\
...
python ${main_program} --proj_name ${proj_name} --main_program ${main_program} --task ${task} \
    --model ${model} --dataset_name ${dataset} --sonn_type ${sonn_type} --npoints ${npoints} \
